...
def run_prune(
    ...
    print("=" * 94)
    print(f"|\t{'layer name':<28}{'|':<10}{'origin channels':<20}{'|':<10}{'remaining channels':<20}|")
    remain_num = 0
    ...
    pruned_yaml["backbone"] = [
        ...
        [-1, 1, GhostConvPruned, [128, 3, 2]],  # 1-P2/4
        [-1, 3, C3Pruned, [128]],
        [-1, 1, GhostConvPruned, [256, 3, 2]],  # 3-P3/8
        [-1, 6, C3Pruned, [256]],
        [-1, 1, GhostConvPruned, [512, 3, 2]],  # 5-P4/16
        [-1, 9, C3Pruned, [512]],
        [-1, 1, GhostConvPruned, [1024, 3, 2]],  # 7-P5/32
        [-1, 3, C3Pruned, [1024]],
        [-1, 1, SPPFPruned, [1024, 5]],  # 9
    ]
    pruned_yaml["head"] = [
        [-1, 1, Eca_layer, [1024, 7]],
    
        [-1, 1, GhostConvPruned, [512, 1, 1]],
        [-1, 1, nn.Upsample, [None, 2, 'nearest']],
        [6, 1, Eca_layer, [512, 7]],
        [[-2, -1], 1, Concat, [1]],  # cat backbone P4
        [-1, 3, C3Pruned, [512, False]],  # 15
    
        [-1, 1, GhostConvPruned, [256, 1, 1]],
        [-1, 1, nn.Upsample, [None, 2, 'nearest']],
        [4, 1, Eca_layer, [256, 5]],
        [[-2, -1], 1, Concat, [1]],  # cat backbone P3
        [-1, 3, C3Pruned, [256, False]],  # 20 (P3/8-small)
    
        [-1, 1, GhostConvPruned, [256, 3, 2]],
        [[-1, 16], 1, Concat, [1]],  # cat head P4
        [-1, 3, C3Pruned, [512, False]],  # 23 (P4/16-medium)
    
        [-1, 1, GhostConvPruned, [512, 3, 2]],
        [[-1, 11], 1, Concat, [1]],  # cat head P5
        [-1, 3, C3Pruned, [1024, False]],  # 26 (P5/32-large)
    ...
    # ============================================================================== #
    maskbndict = {}
    ghost_list=['model.1.cv2', 'model.3.cv2', 'model.5.cv2', 'model.7.cv2', 'model.11.cv2', 'model.16.cv2', \
                'model.21.cv2', 'model.24.cv2' ]
    for bnname, bnlayer in model.named_modules():
        if isinstance(bnlayer, nn.BatchNorm2d):
            ...
            if bnname[:-3] in ghost_list:
                mask = maskbndict[bnname[:-4]+'1.bn']
            ...
            print(f"|\t{bnname:<28}{'|':<10}{bn_module.weight.data.size()[0]:<20}{'|':<10}{int(mask.sum()):<20}|")
            assert int(
    ...
    for ((layername, layer), (pruned_layername, pruned_layer)) in zip(model.named_modules(),
        ...
        if isinstance(layer, nn.Conv2d) and not layername.startswith("model.27") and layername[:-5] not in ghost_list:
        ...
        if isinstance(layer, nn.Conv2d) and layername.startswith("model.27"):
        ...
        if isinstance(layer, nn.Conv1d):
            ...
            changed_state.append(layername + ".weight")
        if isinstance(layer, nn.Conv2d) and layername[:-5] in ghost_list:
#             former = from_to_map[convname]
            out_idx = np.squeeze(np.argwhere(np.asarray(maskbndict[layername[:-4] + "bn"].cpu().numpy())))
            # in_idx = np.squeeze(np.argwhere(np.asarray(maskbndict[former].cpu().numpy())))
            in_idx = 1            
    ...
    if not training:
        if pt and not single_cls:  # check --weights are trained on --data
            ...
            assert ncm == nc, f'{weights} ({ncm} classes) trained on different --data than what you passed ({nc} ' \
                              f'classes). Pass correct combination of --weights and --data that are trained together.'
        if device.type != 'cpu':
            model(torch.zeros(1 if pt else batch_size, 3, imgsz, imgsz).to(device).type_as(next(model.parameters())))  # run once
        # model.warmup(imgsz=(1 if pt else batch_size, 3, imgsz, imgsz))  # warmup
        pad, rect = (0.0, False) if task == 'speed' else (0.5, pt)  # square inference for benchmarks
        task = task if task in ('train', 'val', 'test') else 'val'  # path to train/val/test images
        dataloader = create_dataloader(data[task],
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/UAVVaste.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/train/exp-train-sparity/weights/last.pt', help='model path(s)')
    parser.add_argument('--cfg', type=str, default=ROOT / 'models/yolov5n-ECA-FPN-Ghost.yaml', help='model.yaml path')
    parser.add_argument('--percent', type=float, default=0.25, help='prune percentage')
    ...
    parser.add_argument('--workers', type=int, default=15, help='max dataloader workers (per RANK in DDP mode)')
    parser.add_argument('--single-cls', action='store_true', default=True, help='treat as single-class dataset')
