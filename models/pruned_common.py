...
from utils.general import (LOGGER, check_requirements, check_suffix, check_version, colorstr, increment_path,
                           make_divisible, non_max_suppression, scale_boxes, xywh2xyxy, xyxy2xywh)
...
class GhostConvPruned(nn.Module):
    # Ghost Convolution https://github.com/huawei-noah/ghostnet
    def __init__(self, cv1in, cv1out, cv2out, k=1, s=1, g=1, act=True):  # ch_in, ch_out, kernel, stride, groups
        super().__init__()
        # c_ = cv1out // 2  # hidden channels
        self.cv1 = Conv(cv1in, cv1out, k, s, None, g, act=act)
        self.cv2 = Conv(cv1out, cv2out, 5, 1, None, cv2out, act=act)
