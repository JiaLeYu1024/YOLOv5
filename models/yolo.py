...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
                ...
                BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, conv_bn_relu_maxpool, Shuffle_Block}:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C3, C3TR, C3Ghost, C3x, conv_bn_relu_maxpool, Shuffle_Block}:
...
def parse_pruned_model(maskbndict, d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv]:
            ...
            c2 = cv1out+cv1out
            args = [cv1in, cv1out, cv1out, *args[1:]]

            from_to_map[named_m_cv1_bn] = fromlayer[f]
            from_to_map[named_m_cv2_bn] = named_m_cv1_bn
            inputtmp = []
            inputtmp.append(named_m_cv1_bn)
            inputtmp.append(named_m_cv2_bn)
...
#             inputtmp = [fromlayer[x] for x in f]
            ...
            for x in f:
                if isinstance(fromlayer[x], list):
                    for tmp in fromlayer[x]:
                        inputtmp.append(tmp)
                else:
                    inputtmp.append(fromlayer[x])
            fromlayer.append(inputtmp)
        elif m is Detect:
